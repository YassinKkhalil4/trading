...
class ProductionScannerEngine:
    ...
    def run_once(self, symbols: list[str] | None = None) -> ProductionScannerRunResult:
        ...
        for symbol in symbols:
            checks = [
                self._vwap_reclaim(symbol),
                self._post_earnings_continuation(symbol),
                self._opening_range_breakout(symbol),
                self._news_momentum(symbol),
                self._catalyst_run_up(symbol),
                self._relative_strength(symbol),
                self._sector_leadership(symbol),
            ]
